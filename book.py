...
def show(book_id):
    ...
    if current_user.is_authenticated:
        ...
        review = False
    if Review.query.filter_by(book_id=book_id).all():
        reviews = Review.query.filter_by(book_id=book_id).all()
        if review:
            reviews.remove(review)
    else:
        reviews = False
    return render_template('book/show.html', book=book, book_genre=book_genre, img=img, review=review, reviews=reviews)
